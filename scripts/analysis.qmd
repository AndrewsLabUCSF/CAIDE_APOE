...
dat <- read_tsv(dat_path) %>% 
  ...
  mutate(
    ...
    m2caide_apoe = fct_relevel(m2caide_apoe, "mid_e3/e3") 
  )
```
...
tble1 = dat %>% 
  ...
  #               caide_apoe,naccudsd) %>%
  dplyr::select(race, apoe, age, gender, i_educ, i_bmi,
                i_bpsys,i_hypchol, caide, caide_missing, caide_cat,
                caide_apoe,naccudsd) %>%
  ...
  naccudsd = factor(naccudsd, levels = c("CU", "MCI", "ADRD"))
  ...
  text_case_match(
    ...
    "gender"~ "Gender",
    "i_bmi"~"BMI",
    "i_bpsys" ~"Hypertension",
    "i_hypchol"~ "Hypercholesterolemia",
    "caide"~ "CAIDE",
    "caide_missing"~ "CAIDE Missing",
    "caide_cat"~ "CAIDE",
    "caide_apoe"~"CAIDE x APOE"
  ) 
...
apoe_geno_res_p <- apoe_geno_res %>%
    ...
    #   term =  "e3/e3",
    # ) %>%
  mutate(race = fct_relevel(race, 'Black', 'Latinx', 'Asian',  'NLW'), 
...
ggsave(here("results/figures/joint_apoe_race_all_bl.png"), plot = apoe_geno_res_p, width = 9, height = 5, units = 'in')
...
joint_caid_apoe_all_bl_p <- ggplot(caide_res.df, aes(x = or, y = race, color = race)) + 
    ...
    scale_y_discrete(limits=rev, labels = c("NLW", "Asian", "Latinx", "Black")) + 
    scale_color_manual(
      labels = c("Black", "Latinx", "Asian", "NLW"),
...
joint_caid_apoe_all_bl_p
ggsave(here("results/figures/joint_caid_apoe_all_bl.png"), plot = joint_caid_apoe_all_bl_p, width = 9, height = 4, units = 'in')
...
joint_caid_apoe_adrd_mci_bl_p
ggsave(here("results/figures/joint_caid_apoe_adrd_mci_bl.png"), plot = joint_caid_apoe_adrd_mci_bl_p, width = 9, height = 3.5, units = 'in')
...
ggsave(here("results/figures/joint_caide_apoe_cat_all_bl.png"), plot = joint_caide_apoe_cat_all_bl, width = 9, height = 4, units = 'in')
...
ggsave(here("results/figures/joint_caide_apoe_adrd_mci_bl.png"), plot = joint_caide_apoe_adrd_mci_bl_p, width = 9, height = 3.5, units = 'in')
...
joint_mcaid_apoe_adrd_mci_bl_p
ggsave(here("results/figures/joint_mcaid_apoe_adrd_mci_bl.png"), plot = joint_mcaid_apoe_adrd_mci_bl_p, width = 9, height = 3.5, units = 'in')
...
```{r}
## Combined plot
legend_f1 <- cowplot::get_legend(joint_mcaid_apoe_adrd_mci_bl_p + theme(strip.text = element_blank()))

legend_f1 <- get_plot_component(joint_mcaid_apoe_adrd_mci_bl_p + theme(strip.text = element_blank()), 'guide-box-bottom', return_all = TRUE)

outfile_poster.path = '~/Downloads/joint_caide_mcaide_adrd_mci.png'
# tiff(outfile_neuron.path, width = 9, height = 4.5, units = "in", res = 300)
png(outfile_poster.path, width = 5, height = 3, units = "in", res = 600)
png("results/figures/figure1.png", width = 3.5, height = 3.5, units = "in", res = 600)
...
fig1_diff <- bind_rows(
  caide_df_pval %>% filter(outcome == "ADRD + MCI"), 
  mcaide_df_pval %>% filter(outcome == "ADRD + MCI") %>% filter(term == "mCAIDE")
)

fig1.df <- bind_rows(
  caide_res.df %>% filter(outcome == "ADRD + MCI"),
  mcaide_res_df %>% filter(outcome == "ADRD + MCI") %>% filter(term == "mCAIDE")
)

theme.size = 8

figure1.p <- ggplot(fig1.df, aes(y = or, x = race, color = race)) + 
    facet_grid(cols = vars(term), rows = vars(outcome), scales = 'fixed') + 
    coord_cartesian(ylim=c(0.5, 5.3)) + 
    geom_hline(yintercept = 1, linetype = 2) + 
    geom_linerange(aes(ymin = or_lci, ymax = or_hci), linewidth = 0.5) + 
    geom_point(size = 0.75) + 
    ggprism::add_pvalue(fig1_diff,
                        bracket.nudge.y = 4.2,
                        label = "{stars}", colour = 'black',
                        tip.length = 0.01, bracket.size = 0.2) +
    # scale_x_continuous("Odds Ratio", trans = 'log', 
...
ggsave(here(file.path("results/figures/figure1_new.png")), plot = figure1.p, units = 'in', width = 4, height = 2)
ggsave(here(file.path("results/figures/figure1_new.tiff")), plot = figure1.p, units = 'in', width = 4, height = 2)
...
ggsave(here("results/figures/figure3.png"), plot = figure3, width = 6, height = 3.5, units = 'in')
ggsave(here("results/figures/figure3.tiff"), plot = figure3, width = 6, height = 3.5, units = 'in')
```
...
figure2 <- ggplot(sex_df, aes(x = or, y = race, color = race)) + 
  facet_grid(cols = vars(term), rows = vars(outcome)) + 
  geom_vline(xintercept = 1, linetype = 2) + 
  geom_point() + 
  geom_linerange(aes(xmin = or_lci, xmax = or_hci), linewidth = 0.75) + 
  ...
  scale_y_discrete(labels = c("Black", "Latinx", "Asian", "NLW"))+
  theme_bw() + 
  theme(
    text = element_text(size = theme.size),
    axis.title.y = element_blank(),
...
ggsave(here("results/figures/figure2.png"), plot = figure2, width = 6, height = 3.5, units = 'in')
ggsave(here("results/figures/figure2.tiff"), plot = figure2, width = 6, height = 3.5, units = 'in')
```
...
r01_adrd_mci_diff <- caide_df_pval %>% 
  bind_rows(mcaide_df_pval) %>%
  filter(outcome == "ADRD + MCI") %>%
  filter(term %in% c('CAIDE', 'mCAIDE'))

mcaide_caide.df <- caide_res.df %>%
  bind_rows(mcaide_res_df) %>%
  filter(outcome == "ADRD + MCI") %>%
  filter(term %in% c('CAIDE', 'mCAIDE'))
...
caide_mcaide_adrd_mci_bl_p <- ggplot(mcaide_caide.df, aes(y = or, x = race, color = race)) + 
    facet_grid(cols = vars(term), rows = vars(outcome), scales = 'free') + 
    coord_cartesian(ylim=c(0.5, 3.3)) + 
    geom_hline(yintercept = 1, linetype = 2) + 
    geom_linerange(aes(ymin = or_lci, ymax = or_hci), linewidth = 0.5) + 
    geom_point(size = 0.75) + 
    ggprism::add_pvalue(r01_adrd_mci_diff,
                        bracket.nudge.y = 2.1,
                        label = "{stars}", colour = 'black',
                        tip.length = 0.01, bracket.size = 0.2) +
    # scale_x_continuous("Odds Ratio", trans = 'log', 
...
ggsave("~/Downloads/caide_mcaide_adrd_mci_bl.png", plot = caide_mcaide_adrd_mci_bl_p, height = 2, width = 2.5, units = 'in')
...
f2.df <- bind_rows(
  ...
  mutate(
    ...
    term = str_replace_all(term, "e3", "ε3"),
    term = str_replace_all(term, "e4", "ε4"), 
    term = str_replace_all(term, " \\(reference\\)", ""), 
    term = fct_relevel(term, "Favourable, ε2+", "Intermediate, ε2+", "Unfavourable, ε2+",
                       "Favourable, ε3/ε3", "Intermediate, ε3/ε3", "Unfavourable, ε3/ε3",
                       "Favourable, ε4+", "Intermediate, ε4+", "Unfavourable, ε4+"),
    race = str_replace_all(race, "NHW", "NLW"), 
  )
...
png("results/figures/r01_figure2.png", width = 5.5, height = 2.5, units = 'in', res = 300)
pageCreate(width = 5.5, height = 2.5, default.units = "inches")

plotGG(
  plot = figure2,
  x = 0, y = 0,
  width = 5.5, height = 2.5, just = c("left", "top")
)
...
figure6 <- f6.df  %>% 
  ggplot(., aes(x = or, y = term, color = race, shape = reference)) + 
    facet_grid(cols = vars(race),  scales = "free_x") + 
    geom_vline(xintercept = 1, linetype = 2) + 
    geom_point() + 
    geom_linerange(aes(xmin = or_lci, xmax = or_hci), linewidth = 0.75) + 
    scale_x_continuous("OR (95% CI)", trans = 'log',
...
png("results/figures/r01_figure6.png", width = 5.5, height = 1.25, units = 'in', res = 300)
pageCreate(width = 5.5, height = 1.25, default.units = "inches")

plotGG(
  plot = figure6,
  x = 0, y = 0,
  width = 5.5, height = 1.25, just = c("left", "top")
)
...
#| echo: false
data_wo_i = dat %>% select(-starts_with("i_"))

data_wo_i %>% select(everything()) %>%  
summarise(across(everything(), ~sum(is.na(.)/length(.) *100))) %>% 
mutate(across(where(is.numeric), ~ round(.x, digits = 2))) %>% 
select(bmi, hypchol, educ , hyperten) %>% 
rowwise() %>% 
pivot_longer(cols = everything(), names_to = "names", values_to = "values")%>%
gt() %>% 
cols_label(names~"Variable", values~"Percent NA")
...
```{r}
#do complete case analysis on the 4 specific variables. 
cc_dat = dat %>% filter(complete.cases(bmi,hyperten, hypchol, educ))


mod_fitsCC <- function(data,i){
  mcaide_full = glm(dx ~ apoe + z_caide, data = data, family = 'binomial')
  caide_full  = glm(dx ~ apoe + z_mcaide, data = data, family = 'binomial')
  reduced_model = glm(dx ~ apoe, data = data, family = 'binomial')
  allM = list(caide_full, mcaide_full,reduced_model)
...
cc_res <- bind_rows(
  cc_dat %>%
    ...
    mutate(
      outcome = "ADRD + MCI",
      res_mcaide_full = map(data, mod_fitsCC,1),
      res_caide_full = map(data, mod_fitsCC, 2),
      res_red = map(data, mod_fitsCC,3)
    ), 
  cc_dat %>%
    filter(naccudsd %nin% c(3)) %>%
    group_by(race) %>%
...
) %>% mutate(fit_mcaide_full = map(res_mcaide_full, glance),
             df_mcaide_full = map(res_mcaide_full, tidy), 
        #      pred_mcaide_full = map2(res_mcaide_full, data, predict, type =
        #                                'response'),
        #      actual_mcaide_full = map(data, ~.x$dx),
        # auc_mcaide = map2_dbl(actual_mcaide_full, pred_mcaide_full, ~
        #                         auc(roc(.x, .y))),
        #      roc_mcaide = map2(actual_mcaide_full, pred_mcaide_full, ~roc(.x,
        #                                                                   .y)),
            
             #model2
             fit_caide = map(res_caide_full, glance),
             df_caide= map(res_caide_full, tidy), 
             # pred_caide = map2(res_caide_full, data, predict, type =
             #                     'response'),
             # caide_actual_vals = map(data, ~.x$dx),
             # auc_caide = map2_dbl(caide_actual_vals, pred_caide, ~ auc(roc(.x,
             #                                                              .y))),
             # roc_caide = map2(caide_actual_vals, pred_caide, ~roc(.x,.y)),
             # 
            #model Red
            fit_red = map(res_red, glance),
            df_red= map(res_red, tidy),
            # pred_red = map2(res_red, data, predict, type = 'response'),
            # actual_red_vals = map(data, ~.x$dx),
            # auc_red = map2_dbl(actual_red_vals, pred_red, ~ auc(roc(.x, .y))),
        
       ) %>% 
    select(-data)

cc_res %>% select(race, outcome, df_mcaide_full) %>% 
  unnest(cols = df_mcaide_full) %>%
  ungroup() %>%
  filter(str_detect(term, 'apoe|mcaide')) %>%
  mutate(
    lci = estimate - (std.error * 1.96),
...
```{r}
#the auc for the complete case analysis
allCC = cc_res %>%
  select(race, outcome,auc_red,
         auc_caide, auc_mcaide, deLong_stat,deLong_pval
         ) %>%
  group_by(race, outcome) %>%
  ungroup() %>%
  mutate(
        across(where(is.numeric), ~ round(.x, digits = 4)),
        deLong_pval = format(deLong_pval, scientific = TRUE, digits = 4),
        #deLong_stat = round(deLong_stat, 3)
         ) %>%
  gt() %>%
  cols_label(auc_red ~"AUC M1",
             auc_caide ~ "AUC M2",
             auc_mcaide ~ "AUC M3",
             deLong_stat ~ "Statistics",
             deLong_pval ~"p-value"
             ) %>%
  tab_footnote(footnote = html( "M1: Y<sub>dx</sub> ~ &beta;<sub> APOE</sub>"
                               ),
               locations = cells_column_labels(columns = c(auc_red))
               ) %>%
   tab_footnote(footnote = html( "M2: Y<sub>dx</sub> ~ &beta;<sub> APOE</sub>+
                                  &beta; <sub>z_caide</sub>"),
               locations = cells_column_labels(columns = c(auc_caide))
               ) %>%
   tab_footnote(footnote = html( "M3: Y<sub>dx</sub> ~ &beta;<sub> APOE</sub>+
                                  &beta;<sub> z_mcaide</sub>"),
               locations = cells_column_labels(columns = c(auc_mcaide))
               )

allCC
```

## indiv model for caide

-   run the components of ciade seperatley. Example y\~ bp + chol + apoe4 + cohor etc report model results. mcaide = sum(mcaide_age, mcaide_educ, mcaide_sex, mcaide_bmi, mcaide_sbp, mcaide_chol, na.rm = F),

```{r}
individ_mod_fit <- function(data){
  glm(dx ~ apoe + cohort + age + educ + gender +
        bmi + i_bpsys + i_hypchol , data = data, family = 'binomial')

}
indiv_mod_res <- bind_rows(
  dat %>%
    group_by(race) %>%
    nest() %>%
    mutate(
      outcome = "ADRD + MCI",
      res = map(data, individ_mod_fit), 
    ), 
  dat %>%
    filter(naccudsd %nin% c(3)) %>%
    group_by(race) %>%
  ...
  mutate(
    ...
    df = map(res, tidy), 
    pred = map2(res, data, predict, type ='response'),
    actual = map(data, ~.x$dx),
    auc= map2_dbl(actual, pred, ~auc(roc(.x, .y))),
    roc = map2(actual, pred, ~roc(.x,.y)),
  ) %>%
  select(-data)


auc_indiv = indiv_mod_res %>%  select(race, outcome, auc) %>% 
  group_by(race, outcome) %>% 
  ungroup() %>% 
  mutate(across(where(is.numeric), ~ round(.x, digits = 4))) %>% 
  gt() %>%cols_label(auc ~"AUC") %>% 
  tab_footnote(footnote = html( "Y<sub>dx</sub> ~ &beta;<sub> APOE</sub>+ 
                                &beta;<sub> cohort</sub> + &beta;<sub>age</sub>+
                                &beta;<sub> education </sub> +
                                &beta;<sub> gender </sub> + 
                                &beta;<sub> BMI </sub> + 
                                &beta;<sub> hypercholesteremia</sub> + 
                                &beta;<sub> systolic BP </sub>
                                "),
               locations = cells_column_labels(columns = c(auc)) )


indiv_mod_res_table <- indiv_mod_res %>%
  select(race, outcome, df) %>%
  unnest(cols = df) %>%
  ungroup() %>%
  filter(!str_detect(term, 'Intercept')) %>%
  mutate(
    ...
    or_hci = exp(hci),
    term = str_replace(term, "apoe", ""),
    term = fct_relevel(term, 'e2+', 'e4+'),
    race = fct_relevel(race, "Black", "Hispanic", "Asian", "NHW"),
    term = fct_recode(term, "ε2+" = "e2+", "ε4+" = "e4+"),
    # term = str_replace(term, "apoe", ""), 
    # term = fct_relevel(term, 'e2+', 'e4+', 'z_caide'),    
    # race = fct_relevel(race, "Black", "Hispanic", "Asian", "NHW"), 
    # term = fct_recode(term, "ε2+" = "e2+", "ε4+" = "e4+", "CAIDE" = "z_caide")
  ...
  rowwise() %>% 
  mutate_at(vars(-race, -outcome, -term,-p.value), ~ round(.x, digits = 3))

indiv_mod_res_table %>% gt()
```

## delong indiv model

```{r}
individ_mods <- function(data,i){
  m1 = glm(dx ~ apoe + cohort + age + educ + gender +bmi + i_bpsys + i_hypchol , data = data, family = 'binomial')
  m2 = glm(dx ~ apoe + z_caide + cohort, data = data, family = 'binomial')
...
indiv_mod_res <- bind_rows(
  dat %>%
    ...
    mutate(
      outcome = "ADRD + MCI",
      res_ind = map(data, individ_mods,1),
      res_caide = map(data,individ_mods,2),
    ), 
  dat %>%
    filter(naccudsd %nin% c(3)) %>%
    group_by(race) %>%
  ...
  mutate(
    fit_indiv = map(res_ind, glance),
    df_indiv = map(res_ind, tidy), 
    pred_indiv = map2(res_ind, data, predict, type ='response'),
    actual_indiv = map(data, ~.x$dx),
    auc_indiv = map2_dbl(actual_indiv, pred_indiv, ~ auc(roc(.x, .y))),
    roc_indiv = map2(actual_indiv, pred_indiv, ~roc(.x, .y)),
    
    #model2
    fit_caide = map(res_caide, glance),
    df_caide= map(res_caide, tidy), 
    pred_caide = map2(res_caide, data, predict, type = 'response'),
    caide_actual_vals = map(data, ~.x$dx),
    auc_caide = map2_dbl(caide_actual_vals, pred_caide, ~ auc(roc(.x, .y))),
    roc_caide = map2(caide_actual_vals, pred_caide, ~roc(.x,.y)),
    
    
    deLong_pval = map2(roc_indiv, roc_caide, ~roc.test(.x,.y,method =
                                                          "delong")[["p.value"]]),
    deLong_stat = map2(roc_indiv, roc_caide, ~roc.test(.x,.y,method =  
                                                          "delong")[["statistic"]][[1]]),
  ) %>%
  select(-data)

auc_i_c = indiv_mod_res %>% 
  select(race, outcome,auc_indiv, 
         auc_caide, deLong_stat,deLong_pval
  ) %>% 
  group_by(race, outcome) %>% 
  ungroup() %>% 
  mutate(across(where(is.numeric), ~ round(.x, digits = 4)),
         deLong_pval = format(deLong_pval, scientific = TRUE, digits = 3),
         #deLong_stat = round(deLong_stat, 3)
  ) %>% 
  gt() %>% 
  cols_label(auc_indiv ~"AUC M1",
             auc_caide ~ "AUC M2",
             deLong_stat ~ "Statistic",
             deLong_pval~ "p.value") %>% 
  tab_footnote(footnote = html("M1: Y<sub>dx</sub> ~ &beta;<sub> APOE</sub>+ 
                                &beta;<sub> cohort</sub> + &beta;<sub>age</sub>+
                                &beta;<sub> education </sub> +
                                ...
                                &beta;<sub> systolic BP </sub>
                                "),
               locations = cells_column_labels(columns = c(auc_indiv))
  ) %>% 
  tab_footnote(footnote = html( "M2: Y<sub>dx</sub> ~ &beta;<sub> APOE</sub>+ 
                                  &beta; <sub>z_caide</sub>+ &beta;<sub> cohort</sub>"),
               locations = cells_column_labels(columns = c(auc_caide))
  ) %>% 
  tab_footnote(footnote = html("Based on DeLong's method"),
               locations = cells_column_labels(columns = c(deLong_stat, deLong_pval))
  ) 
auc_i_c
```
...
caide_mod_fit <- function(data){
  #glm(dx ~ apoe + z_caide + cohort, data = data, family = 'binomial')
  glm(dx ~ apoe + z_caide + cohort +apoe*z_caide, data = data, family = 'binomial')

}
caide_interaction_res <- bind_rows(
  dat %>%
    group_by(race) %>%
    nest() %>%
    mutate(
...
caide_intera_res.df <- caide_interaction_res %>%
  select(race, outcome, df) %>%
  unnest(cols = df) %>%
  ungroup() %>%
  filter(str_detect(term, 'apoe|caide|cohort')) %>%
  mutate(
    p.value = format(p.value, scientific = TRUE, digits = 3),
    lci = estimate - (std.error * 1.96),
    hci = estimate + (std.error * 1.96), 
...
mcaide_mod_fit <- function(data){
  #glm(dx ~ apoe + z_caide + cohort, data = data, family = 'binomial')
  glm(dx ~ apoe + z_mcaide + cohort +apoe*z_mcaide, data = data, family = 'binomial')

}
mcaide_interaction_res <- bind_rows(
  dat %>%
    group_by(race) %>%
    nest() %>%
    mutate(
...
mcaide_intera_res.df <- mcaide_interaction_res %>%
  select(race, outcome, df) %>%
  unnest(cols = df) %>%
  ungroup() %>%
  filter(str_detect(term, 'apoe|caide')) %>%
...
mcaide_intera_res.df %>% gt()
```

## All AUCs

Obtain AUC for the following models: m1) dx \~ cohort + z_mcaide+ apoe m2) dx \~ cohort + z_caide+ apoe

use Delong method

```{r}
library(pROC)
mod_fits <- function(data,i){
  mcaide_full = glm(dx ~ apoe + z_caide + cohort, data = data, family = 'binomial')
  caide_full  = glm(dx ~ apoe + z_mcaide + cohort, data = data, family = 'binomial')
  reduced_model = glm(dx ~ apoe  + cohort, data = data, family = 'binomial')
  allM = list(caide_full, mcaide_full,reduced_model)
  
  return(allM[[i]])
}

all_res_AUC <- bind_rows(
  dat %>%
    group_by(race) %>%
    nest() %>%
    mutate(
      outcome = "ADRD + MCI",
      res_mcaide_full = map(data, mod_fits,1),
      res_caide_full = map(data, mod_fits, 2),
      res_red = map(data, mod_fits,3)
    ), 
  dat %>%
    filter(naccudsd %nin% c(3)) %>%
    group_by(race) %>%
    ...
    mutate(
      ...
      res_red = map(data, mod_fits,3)
             ...
             pred_mcaide_full = map2(res_mcaide_full, data, predict, type =
                                       'response'),
             actual_mcaide_full = map(data, ~.x$dx),
        auc_mcaide = map2_dbl(actual_mcaide_full, pred_mcaide_full, ~ auc(roc(.x, .y))),
             roc_mcaide = map2(actual_mcaide_full, pred_mcaide_full, ~roc(.x, .y)),
            
             #model2
             fit_caide = map(res_caide_full, glance),
             df_caide= map(res_caide_full, tidy), 
             pred_caide = map2(res_caide_full, data, predict, type = 'response'),
             caide_actual_vals = map(data, ~.x$dx),
             auc_caide = map2_dbl(caide_actual_vals, pred_caide, ~ auc(roc(.x, .y))),
             roc_caide = map2(caide_actual_vals, pred_caide, ~roc(.x,.y)),
            ...
            auc_red = map2_dbl(actual_red_vals, pred_red, ~ auc(roc(.x, .y))),
             ...
             #auc diff
        deLong_pval = map2(roc_mcaide, roc_caide, ~roc.test(.x,.y,method =
                                               "delong")[["p.value"]]),
        deLong_stat = map2(roc_mcaide, roc_caide, ~roc.test(.x,.y,method =  
                                               "delong")[["statistic"]][[1]]),
       ) %>% 
    select(-data)
```
...
all_AUC = all_res_AUC %>% 
  ...
  tab_footnote(footnote = html( "M1: Y<sub>dx</sub> ~ &beta;<sub> APOE</sub>+ 
   ...
   tab_footnote(footnote = html( "M3: Y<sub>dx</sub> ~ &beta;<sub> APOE</sub>+ 
                                  &beta;<sub> z_mcaide</sub>+ &beta;<sub> cohort</sub>"),
               locations = cells_column_labels(columns = c(auc_mcaide))
               ) %>% 
  tab_footnote(footnote = html("Based on DeLong's method using M2 & M3"),
               locations = cells_column_labels(columns = c(deLong_stat, deLong_pval))
               ) 
...
```{r}
mod_fits <- function(data,i){
 glm(dx ~ m2caide_apoe + cohort, data = data, family = 'binomial')
}
  ...
  dat %>%
    ...
    mutate(
      ...
      res2 = map(data, mod_fits),
             ...
             fit2 = map(res2, glance),
             df2 = map(res2, tidy), 
       ) %>% 
    select(-data)

results_m2 = all_res %>% select(race, outcome, df2) %>% 
  unnest(cols = df2) %>%
  ungroup() %>%
  filter(str_detect(term, 'm2caide')) %>%
  bind_rows(add_me) %>%
  mutate(
    lci = estimate - (std.error * 1.96),
    hci = estimate + (std.error * 1.96), 
    ...
    or_lci = exp(lci),
    or_hci = exp(hci),
    term = str_replace(term, "m2caide_apoe", ""), 
     term = fct_relevel(term, 
                       "high_e4+", "mid_e4+", "low_e4+", 
                       "high_e3/e3", "mid_e3/e3",  "low_e3/e3", 
                       "high_e2+", "mid_e2+", "low_e2+",
    ...
    term = fct_recode(term, 
                      "Unfavourable, e4+" = "high_e4+", 
                      "Intermediate, e4+" = "mid_e4+", 
                      "Favourable, e4+" = "low_e4+", 
                      "Unfavourable, e3/e3" = "high_e3/e3", 
                      "Intermediate, e3/e3 (reference)" = "mid_e3/e3", 
                      "Favourable, e3/e3" = "low_e3/e3", 
                      "Unfavourable, e2+" = "high_e2+", 
                      "Intermediate, e2+" = "mid_e2+", 
                      "Favourable, e2+" = "low_e2+" 
    ),
    reference = ifelse(or == 1, TRUE, FALSE), 
    race = fct_relevel(race, "Black", "Hispanic", "Asian", "NHW"), 
    race = fct_recode(race, "Latinx" = "Hispanic"), 
    race = fct_recode(race, "NLW" = "NHW")
    # p.value = format(p.value, scientific = TRUE, digits = 3),
    # or = format(or, scientific = TRUE, digits = 3),
    # or_lci = format(or_lci, scientific = TRUE, digits = 3),
    # or_hci = format(or_hci, scientific = TRUE, digits = 3),
  ) 

```

## Figure 1 (supplemental beta plot)

```{r}
#add CAIDE, mCAIDE
betaPlot.df <- bind_rows(
 # filter(caide_apoe_cat_df ,
 #        outcome == "ADRD + MCI") %>%
 #    mutate(model = "CAIDE"),
 #  filter(mcaide_apoe_cat_df ,
 #         outcome == "ADRD + MCI")  %>%
 #    mutate(model = "mCAIDE"),
  filter(results_m2,
          outcome == "ADRD + MCI")  %>%
    mutate(model = "m2CAIDE")
) %>%
  mutate(
    term = str_replace_all(term, "e2", "ε2"), 
    term = str_replace_all(term, "e3", "ε3"),
...
bPlot <- a  %>% 
  ...
  geom_point() + 
  geom_linerange(aes(xmin = or_lci, xmax = or_hci), linewidth = 0.75) + 
  scale_x_continuous("OR (95% CI)", transform = 'log',
                     breaks = c(0.25, 0.5, 1, 2, 4, 8, 16, 32),
                     labels = c(0.25, 0.5, 1, 2, 4, 8, 16, 32),
                     limits = c(0.15, 9),
                     oob=scales::rescale_none
...
ggsave(here("results/figures/supplements_fig1.png"), plot = bPlot, width = 6, height = 3.5, units = 'in')

```

## tertiles ranges unf, favorable etc
...
dat %>%
  group_by(mcaide_cat) %>%
  summarize(
    min_value = min(mcaide),
    max_value = max(mcaide),
  )

dat %>%
  group_by(m2caide_cat) %>%
  summarize(
    min_value = min(m2caide),
    max_value = max(m2caide),
  )
```
...
```{r}
 a = all_res %>% select(race, outcome,df,df2) %>% 
  unnest(cols = c(df, df2), names_sep = "_") %>%
  ungroup() %>%
  filter(str_detect(df_term, 'apoe|mcaide'),
          str_detect(df2_term, 'apoe|z_m2caide')) %>% 
  select(race, outcome, ends_with("_term"), c(b,b2) = ends_with("_estimate"),
         ends_with("_std.error"),   ends_with("_p.value"))

  pivot_wider(names_from = race, values_from = c(contains("_estimate"),                                          contains("_std.error"),contains("_p.value")))
  mutate(.,
   diff_nhw_bla = fzscore(b_NHW, b_Black, se_NHW, se_Black),
  p_diff_nhw_bla = 2*pnorm(q=abs(diff_nhw_bla), lower.tail=FALSE),
  )

caide_res.df %>% select(race, outcome, term, b = estimate, se = std.error, p = p.value ) %>%
  pivot_wider(names_from = race, values_from = c(b, se, p)) %>%
  mutate(.,
         ## Black vs NHW
         diff_nhw_bla = fzscore(b_NHW, b_Black, se_NHW, se_Black),
