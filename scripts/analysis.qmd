...
dat <- read_tsv("data/joint_df.tsv.gz") %>% 
...
caide_e2_res_p <- filter(caide_res, term == "ε2+") %>% effect_plot(.) 
caide_e4_res_p <- filter(caide_res, term == "ε4+") %>% effect_plot(.) + 
  ggprism::add_pvalue(caide_df_pval %>% filter(term == "ε4+")
                      , bracket.nudge.y = 3, label = "{stars}", colour = 'black', tip.length = 0.01, bracket.size = 0.2) 

caide_res_p <- filter(caide_res, term == "CAIDE") %>% effect_plot(.) + 
...
joint_caid_apoe_adrd_mci_bl_p <- ggplot(caide_res %>% filter(outcome == "ADRD + MCI"), aes(x = or, y = race, color = race)) + 
