...
nacc %>% select(everything()) %>%  # replace to your needs
  summarise(across(everything(), ~sum(is.na(.)/length(.) *100))) %>% 
  mutate(across(where(is.numeric), ~ round(.x, digits = 3))) %>% rowwise() %>% 
  pivot_longer(cols = everything(), names_to = "names", values_to = "values") %>% 
  arrange(desc(values)) %>% gt() %>% cols_label(names~"Variable", values~"Percent NA")

adni %>% select(everything()) %>%  # replace to your needs
  summarise(across(everything(), ~sum(is.na(.)/length(.) *100))) %>% 
  mutate(across(where(is.numeric), ~ round(.x, digits = 3))) %>% rowwise() %>% 
  pivot_longer(cols = everything(), names_to = "names", values_to = "values") %>% 
  arrange(desc(values)) %>% gt() %>% cols_label(names~"Variable", values~"Percent NA")
...
dat %>% select(everything()) %>%  # replace to your needs
  summarise(across(everything(), ~sum(is.na(.)/length(.) *100))) %>% 
  mutate(across(where(is.numeric), ~ round(.x, digits = 3))) %>% rowwise() %>% 
  pivot_longer(cols = everything(), names_to = "names", values_to = "values") %>% 
  arrange(desc(values)) %>% gt() %>% cols_label(names~"Variable", values~"Percent NA")
...
mcaide_res_AUC <- bind_rows(
 ...
 all_mcaide_AUC = mcaide_res_AUC %>% 
                  ...
                  ungroup() %>% 
                  rowwise() %>%
                  mutate(auc_diff_full_ac = auc_mcaide_full- auc_mcaide_ac,
                         auc_diff_full_apoe = auc_mcaide_full -auc_mcaide_apoe,
                         auc_diff_ac_apoe = auc_mcaide_ac - auc_mcaide_apoe
                  ) %>%
                  ungroup() %>%
                  mutate(across(where(is.numeric), ~ round(.x, digits = 4)))
```
...
auc_mcaide_table = all_mcaide_AUC %>% gt() %>% 
  cols_label(auc_mcaide_ac	~ html("AUC M2"),
    ...
    tab_footnote(footnote = html( "M1: Y<sub>dx</sub> ~ &beta;<sub> APOE</sub>+ 
                                  &beta;<sub> mcaide</sub>+ &beta;<sub> cohort</sub>"),
               locations = cells_column_labels(columns = c(auc_mcaide_full))
               ) %>% 
    tab_footnote(footnote = html( "M2: Y<sub>dx</sub> ~  &beta;<sub> mcaide</sub>+
                                   &beta;<sub> cohort</sub>"),
