...
get_na_by_cols(dat)[c(1:13),] %>% gt() 

dat %>% select(total_c, caide_u_chol, mcaide_u_chol, origprot, i_total_c,
...
tab_caide <- dat %>% 
  dplyr::select(race, apoe, caide_age, caide_sex, caide_educ, caide_bmi, caide_sbp,
                i_caide_chol, caide, caide_missing, caide_cat, caide_apoe) %>%
  tbl_summary(by = race,
              statistic = list(all_continuous() ~ "{mean} ({sd})")) %>%
  add_n() 
...
dat %>% 
  dplyr::select(race, apoe, caide_age, caide_sex, caide_educ, caide_bmi, caide_sbp,
                i_caide_chol, caide, caide_missing, caide_cat, caide_apoe,naccudsd) %>%
...
get_AUC_vals_by_race = function(df, r){
  
  new_df <- df %>% filter(race == r) %>%
            filter(naccetpr %in% c(1, 88)) %>%
            filter(naccudsd %nin% c(3))
  
  glm_model <- glm(dx ~ apoe + age + gender + i_educ + i_bmi + i_bpsys + i_hypchol, 
                   data = new_df, family = 'binomial')
  
  probabilities <- predict(glm_model, type = "response")
  actuals <- new_df[["dx"]]
    
  auc_val <- auc(roc(actuals, probabilities))
  return(auc_val)
}

get_AUC_vals_by_race(dat, "Hispanic")
get_AUC_vals_by_race(dat, "Asian")
get_AUC_vals_by_race(dat, "Black")
get_AUC_vals_by_race(dat, "NHW")
