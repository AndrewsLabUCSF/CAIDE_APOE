...
tble1 = dat %>% 
  ...
  gt::gtsave("/Users/anaboeriu/Downloads/tble1.docx")
```
...
mod_fits <- function(data,i){
  ...
  allM = list(caide_full, mcaide_full)
...
```{r}
all_AUC = all_res_AUC %>% 
  select(race, outcome,auc_caide, auc_mcaide, deLong_stat,deLong_pval
         ) %>% 
  group_by(race, outcome) %>% 
  ungroup() %>% 
  mutate(across(where(is.numeric), ~ round(.x, digits = 4)),
