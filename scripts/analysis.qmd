...
library(gtsummary)
library(gt)
full_model_auc = mcaide_res_df %>% select(race, outcome,auc_value) %>% 
  group_by(race, outcome) %>% 
  mutate(auc_value = round(auc_value,2)) %>% distinct()

data.frame(full_model_auc) %>% gt()
...
put above into a function with a full reduced and intermediate model. 
```{r}
...
mcaide_res_AUC <- bind_rows(
  dat %>%
    ...
    mutate(
      outcome = "ADRD + MCI",
      resFull = map(data, mcaide_mod_fits,1),
      res_apoe_c = map(data, mcaide_mod_fits, 2),
      res_apoe = map(data, mcaide_mod_fits,3),
    ), 
  dat %>%
    filter(naccudsd %nin% c(3)) %>%
    group_by(race) %>%
...
) %>% mutate(fit_full = map(resFull, glance),
             df_full = map(resFull, tidy), 
             pred_full = map2(resFull, data, predict, type = 'response'),
             actual_vals_full = map(data, ~.x$dx),
             auc_value_full = map2_dbl(actual_vals_full, pred_full, ~ auc(roc(.x, .y))),
            #model 2 
            fit_apoe_c = map(res_apoe_c, glance),
            df_apoe_c = map(res_apoe_c, tidy), 
            pred_apoe_c = map2(res_apoe_c, data, predict, type = 'response'),
            actual_vals_apoe_c = map(data, ~.x$dx),
            auc_value_apoe_c = map2_dbl(actual_vals_apoe_c, pred_apoe_c, ~ auc(roc(.x,.y))),
            #model3
            fit_apoe = map(res_apoe, glance),
            df_apoe= map(res_apoe, tidy), 
            pred_apoe = map2(res_apoe, data, predict, type = 'response'),
            actual_vals_apoe = map(data, ~.x$dx),
            auc_value_apoe = map2_dbl(actual_vals_apoe, pred_apoe, ~ auc(roc(.x, .y)))
          ) %>% 
          select(-data)
...
all_mcaide_AUC <- mcaide_res_AUC %>%
  select(race, outcome,
         auc_value_full, auc_value_apoe_c,auc_value_apoe) %>%
  ungroup() %>% 
  mutate(across(where(is.numeric), ~ round(.x, digits = 2)))


data.frame(all_mcaide_AUC) %>% gt()
```
