...
- run the components of ciade seperatley. Example
y~ bp + chol + apoe4 + cohor etc report model results. 
...
Obtain AUC for the following models:
 m1) dx ~ cohort + z_mcaide+ apoe
 m2) dx ~ cohort + z_caide+ apoe
...
betaPlot.df$model = "m^{2}CAIDE")
levels(betaPlot.df$model) = c("CAIDE","m<sub>2</sub>CAIDE","mCAIDE" )

a = betaPlot.df %>% filter(race != "LatinX" & term != "Favorable, ε2+")

labels_p1 <- c('m2CAIDE+' = TeX("$m^2CAIDE$"))
bPlot <- betaPlot.df  %>% 
  ggplot(., aes(x = or, y = term, color = race, shape = reference)) + 
...
ggsave(here("results/figures/figure4.png"), plot = bPlot, width = 6, height = 3.5, units = 'in')

ggsave("~/Downloads/fig4.png", plot = bPlot, width = 6, height = 3.5, units = 'in')
