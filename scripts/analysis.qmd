...
all_res <- bind_rows(
  dat %>%
    ...
    mutate(
      outcome = "ADRD + MCI",
      res_mcaide_full = map(data, mod_fits,1),
      res_m2caide_full = map(data, mod_fits, 2),
    ), 
  dat %>%
    filter(naccudsd %nin% c(3)) %>%
    group_by(race) %>%
  ...
  dat %>%
    ...
    mutate(
      ...
      res_m2caide_full = map(data, mod_fits, 2),
             ...
             df_mcaide_full = map(res_mcaide_full, tidy), 
             #model2
             fit_m2caide = map(res_m2caide_full, glance),
             df_m2caide= map(res_m2caide_full, tidy), 
       ) %>% 
    select(-data)

f = all_res %>% select(race, outcome, df_mcaide_full) %>% 
...
f2 = all_res %>% select(race, outcome, df_m2caide) %>% 
  unnest(cols = df_m2caide) %>%
  ungroup() %>%
  filter(str_detect(term, 'apoe|m2caide')) %>%
  mutate(
    lci = estimate - (std.error * 1.96),
    hci = estimate + (std.error * 1.96), 
    or = exp(estimate),
