...
dat <- read_tsv(dat_path) %>% 
  ...
  mutate(
    ...
    mcaide_prs = fct_relevel(mcaide_prs, "mid_mid") 
  )
...
tab_caide <- dat %>% 
  dplyr::select(race, apoe, caide_age, caide_sex, caide_educ, caide_bmi, caide_sbp, i_caide_chol, 
                caide, caide_missing, caide_cat, caide_apoe) %>%
  tbl_summary(by = race,
              statistic = list(all_continuous() ~ "{mean} ({sd})")) %>%
  add_n() 
...
as_gt(tab_caide) %>%
  gt::gtsave(., "results/table_caid.docx")
```
