...
```{r}
cc_dat = data_wo_i %>%filter(complete.cases(.))
...
allCC %>% select(race, outcome, df_mcaide_full) %>% 
  ...
  dat %>%
    ...
    mutate(
      ...
      res_red = map(data, mod_fits,3)
             ...
             df_mcaide_full = map(res_mcaide_full, tidy), 
             pred_mcaide_full = map2(res_mcaide_full, data, predict, type = 'response'),
             actual_mcaide_full = map(data, ~.x$dx),
...
mod_fits <- function(data,i){
  ...
  m2caide_full  = glm(dx ~ z_m2caide, data = data, family = 'binomial')
  allM = list(mcaide_full,m2caide_full)
  
  return(allM[[i]])
}

dat %>% select(race, m2caide_apoe, m2caide) %>% group_by(race,m2caide_apoe)
