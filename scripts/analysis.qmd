...
get_nas = function(df, d, title){
  tbl_nas = df %>% select(everything()) %>%  
  summarise(across(everything(), ~sum(is.na(.)/length(.) *100))) %>% 
  mutate(across(where(is.numeric), ~ round(.x, digits = d))) %>% rowwise() %>% 
...
get_nas(nacc, 2, "NACC missing Values")
get_nas(adni, 2, "ADNI missing Values")
get_nas(dat, 2, "Combined missing Values")
...
caide_mod_fit <- function(data,i){
  
  fullModel <-  glm(dx ~ apoe + z_caide + cohort, data = data, family = 'binomial')
  
  apoe_cohort_model <- glm(dx ~ apoe  + cohort, data = data, family = 'binomial')
  
  apoe_model <- glm(dx~apoe, data = data, family = 'binomial')
  
  all_caide_models <- list(fullModel, apoe_cohort_model, apoe_model)
  
  return(all_caide_models[[i]])
}

caide_res_w_AUC <- bind_rows(
  dat %>%
    group_by(race) %>%
    nest() %>%
    mutate(
      outcome = "ADRD + MCI",
      res_caide_full = map(data, caide_mod_fit,1),
      res_caide_apoe_c = map(data, caide_mod_fit, 2),
      res_caide_apoe = map(data, caide_mod_fit,3),
    ), 
  dat %>%
    filter(naccudsd %nin% c(3)) %>%
    group_by(race) %>%
  ...
  mutate(fit_caide_full = map(res_caide_full, glance),
         df_caide = map(res_caide_full, tidy),
         pred_caide_full = map2(res_caide_full, data, predict, type = 'response'),
         actual_caide_vals_full = map(data, ~.x$dx),
  auc_caide_full = map2_dbl(actual_caide_vals_full, pred_caide_full, ~ auc(roc(.x, .y))),
        
        #model2
        fit_caide_ac_model = map(res_caide_apoe_c, glance),
        df_caide_ac_model = map(res_caide_apoe_c,tidy),
        pred_caide_ac_model = map2(res_caide_apoe_c, data, predict,type="response"),
        true_caide_ac_vals = map(data, ~.x$dx),
        auc_caide_ac = map2_dbl(true_caide_ac_vals, pred_caide_ac_model, ~auc(roc(.x,.y))),
        
        #model3
        fit_caide_apoe_model = map(res_caide_apoe, glance),
        df_caide_apoe_model = map(res_caide_apoe,tidy),
        pred_caide_apoe_model = map2(res_caide_apoe, data, predict,type="response"),
        true_caide_apoe_vals = map(data, ~.x$dx),
      auc_caide_apoe = map2_dbl(true_caide_apoe_vals, pred_caide_apoe_model, ~auc(roc(.x,.y))),
  ) %>% 
  select(-data)

all_caide_AUCs <- caide_res_w_AUC %>%
  select(race, outcome,
         auc_caide_full, auc_caide_ac,auc_caide_apoe) %>%
  group_by(race, outcome)%>% 
  rowwise() %>%
  mutate(auc_diff_full_ac = auc_caide_full- auc_caide_ac,
         auc_diff_full_apoe = auc_caide_full -auc_caide_apoe,
         auc_diff_ac_apoe = auc_caide_ac - auc_caide_apoe
  ) %>%
  ungroup() %>%
  mutate(across(where(is.numeric), ~ round(.x, digits = 4)))

data.frame(all_caide_AUCs)

# library(openxlsx)
# write.xlsx(data.frame(all_caide_AUCs), "/Users/anaboeriu/Downloads/caide.xlsx",asTable = TRUE, overwrite = TRUE)

```
##### tables
```{r}
auc_mcaide_table = all_caide_AUCs %>% gt() %>% 
  cols_label(auc_caide_ac	~ html("AUC M2"),
             auc_caide_full~ html("AUC M1"),
             auc_caide_apoe~ html("AUC M3"),
             
             auc_diff_full_ac	 ~ html("AUC: <br> M1 - M2"),
             
             auc_diff_full_apoe~ html("AUC: <br> M1 - M3"),
             
             auc_diff_ac_apoe ~ html("AUC: <br> M2 - M3")
             ) %>% 
    tab_footnote(footnote = html( "M1: Y<sub>dx</sub> ~ &beta;<sub> APOE</sub>+ 
                                  &beta;<sub> zcaide</sub>+ &beta;<sub> cohort</sub>"),
               locations = cells_column_labels(columns = c(auc_caide_full))
               ) %>% 
    tab_footnote(footnote = html( "M2: Y<sub>dx</sub> ~  &beta;<sub> zcaide</sub>+
                                   &beta;<sub> cohort</sub>"),
               locations = cells_column_labels(columns = c(auc_caide_ac))
               ) %>% 
    tab_footnote(footnote = html( "M3: Y<sub>dx</sub> ~ &beta;<sub>APOE</sub>"),
               locations = cells_column_labels(columns = c(auc_caide_apoe))
               ) %>% tab_header("AUC for CAIDE")
...
mcaide_res_df %>% select(race, outcome,auc_value) %>% 
  group_by(race, outcome) %>% 
  ungroup() %>%
  mutate(auc_value = round(auc_value,2)) %>% distinct() %>% gt()
```
...
Obtain AUC for the following models:
 ...
 m2) dx ~ cohort + apoe
 m3) dx ~ apoe
and calculate the AUC difference ie m1-m2 etc


```{r}
mcaide_mod_fits <- function(data,i){
  
  fullModel <- glm(dx ~ apoe + z_mcaide + cohort, data = data,
                                  family = 'binomial')
  apoe_cohort_model <- glm(dx ~ apoe  + cohort, data = data, family = 'binomial')
  apoe_model <- glm(dx~apoe, data = data, family = 'binomial')
  
  allM <- list(fullModel, apoe_cohort_model, apoe_model)
  
  return(allM[[i]])
}

mcaide_res_AUC <- bind_rows(
  dat %>%
    group_by(race) %>%
    nest() %>%
    mutate(
      outcome = "ADRD + MCI",
      res_mcaide_full = map(data, mcaide_mod_fits,1),
      res_macaide_ac = map(data, mcaide_mod_fits, 2),
      res_mcaide_apoe = map(data, mcaide_mod_fits,3),
    ), 
  dat %>%
    filter(naccudsd %nin% c(3)) %>%
    group_by(race) %>%
  ...
  dat %>%
    ...
    auc_mcaide_full = map2_dbl(actual_mcaide_full, pred_mcaide_full, ~ auc(roc(.x, .y))),
            #model 2 
            fit_apoe_c = map(res_macaide_ac, glance),
            df_apoe_c = map(res_macaide_ac, tidy), 
            pred_apoe_c = map2(res_macaide_ac, data, predict, type = 'response'),
            actual_mcaide_ac = map(data, ~.x$dx),
            auc_mcaide_ac = map2_dbl(actual_mcaide_ac, pred_apoe_c, ~ auc(roc(.x,.y))),
            #model3
            fit_apoe = map(res_mcaide_apoe, glance),
            df_apoe= map(res_mcaide_apoe, tidy), 
            pred_apoe = map2(res_mcaide_apoe, data, predict, type = 'response'),
            actual_vals_apoe = map(data, ~.x$dx),
            auc_mcaide_apoe = map2_dbl(actual_vals_apoe, pred_apoe, ~ auc(roc(.x, .y)))
          ) %>% 
          select(-data)

 all_mcaide_AUC = mcaide_res_AUC %>% 
                  select(race, outcome,
                         auc_mcaide_full,auc_mcaide_ac, auc_mcaide_apoe) %>% 
                  group_by(race, outcome) %>% 
                  ungroup() %>% 
                  rowwise() %>%
                  mutate(auc_diff_full_ac_m = auc_mcaide_full- auc_mcaide_ac,
                         auc_diff_full_apoe_m = auc_mcaide_full -auc_mcaide_apoe,
                         auc_diff_ac_apoe_m = auc_mcaide_ac - auc_mcaide_apoe
                  ) %>%
                  ungroup() %>%
                  mutate(across(where(is.numeric), ~ round(.x, digits = 4)))
```
...
#### tables
```{r}
auc_mcaide_table = all_mcaide_AUC %>% gt() %>% 
  cols_label(auc_mcaide_ac	~ html("AUC M2"),
             auc_mcaide_full~ html("AUC M1"),
             auc_mcaide_apoe~ html("AUC M3"),
             
             auc_diff_full_ac_m	 ~ html("AUC: <br> M1 - M2"),
             auc_diff_full_apoe_m~ html("AUC: <br> M1 - M3"),
             auc_diff_ac_apoe_m ~ html("AUC: <br> M2 - M3")
             ) %>% 
    tab_footnote(footnote = html( "M1: Y<sub>dx</sub> ~ &beta;<sub> APOE</sub>+ 
                                  &beta;<sub> z_mcaide</sub>+ &beta;<sub> cohort</sub>"),
               locations = cells_column_labels(columns = c(auc_mcaide_full))
               ) %>% 
    tab_footnote(footnote = html( "M2: Y<sub>dx</sub> ~  &beta;<sub> z_mcaide</sub>+
                                   &beta;<sub> cohort</sub>"),
               locations = cells_column_labels(columns = c(auc_mcaide_ac))
               ) %>% 
    tab_footnote(footnote = html( "M3: Y<sub>dx</sub> ~ &beta;<sub>APOE</sub>"),
               locations = cells_column_labels(columns = c(auc_mcaide_apoe))
               ) %>% tab_header("AUC for mcaide")
