...
get_nas = function(df, deci, title){
  tbl_nas = df %>% select(everything()) %>%  
  summarise(across(everything(), ~sum(is.na(.)/length(.) *100))) %>% 
  mutate(across(where(is.numeric), ~ round(.x, digits = deci))) %>% 
  rowwise() %>% 
  pivot_longer(cols = everything(), names_to = "names", values_to = "values")%>%
  filter(values>0) %>% 
  filter(names == c("bmi", "hypchol", "edu","hyperten")) %>% 
...
get_nas(data_wo_i, 2, "Joint df NAs")
...
individ_mod_fit <- function(data){
  ...
  #       bmi + caide_sbp+ i_hypchol , data = data, family = 'binomial')
...
#gtsave(auc_mcaide_table, "/Users/anaboeriu/Downloads/mcaide.docx")

# write.xlsx(auc_mcaide_table, "/Users/anaboeriu/Downloads/mcaide.xlsx",asTable = TRUE, overwrite = TRUE)

```
## m2CAIDE
```{r}
dat$m2caide_apoe = as.factor(dat$m2caide_apoe)

mod_fits <- function(data,i){
  #mcaide_full = glm(dx ~ apoe + z_mcaide + cohort, data = data, family = 'binomial')
  #m2caide_full  = glm(dx ~ apoe + z_m2caide + cohort, data = data, family = 'binomial')
  
  #mcaide_full = glm(dx ~ z_mcaide + cohort, data = data, family = 'binomial')
  m2caide_full  = glm(dx ~ m2caide_apoe + cohort, data = data, family = 'binomial')
  allM = list(mcaide_full,m2caide_full)
  
  return(allM[[i]])
}
#med_e3/e3 is reference
dat %>%  filter(race=="Asian",m2caide_apoe == "high_e2+") %>% 
count(dx)
...
all_res <- bind_rows(
  dat %>%
    ...
    mutate(
      outcome = "ADRD + MCI",
      #res = map(data, mod_fits,1),
      res2 = map(data, mod_fits, 2),
    ), 
  dat %>%
    filter(naccudsd %nin% c(3)) %>%
    group_by(race,m2caide_apoe) %>%
    nest() %>%
    mutate(
      outcome = "ADRD",
      #res = map(data, mod_fits,1),
...
f = all_res %>% select(race,m2caide_apoe, outcome, df) %>% 
  unnest(cols = df) %>%
  ungroup() %>%
  filter(str_detect(term, 'apoe|mcaide')) %>%
  mutate(
...
f2 = all_res %>% select(race, apoe, outcome, df2) %>% 
  unnest(cols = df2) %>%
  ungroup() %>%
